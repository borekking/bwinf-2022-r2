...
\\
Seien \(d_n, \ \dots \ , \ d_1\) die Ziffern der gegebenen Zahl von links nach rechts und \(d_n', \ \dots \ , \ d_1'\) die jeweiligen Ziffern der korrekten Loesung. Weiter sind \(a_i\) und \(r_i\) die Anzahl an Segmenten, 
...
\\\\
Gegeben seien die Ziffern der gegebenen Zahl \(d_1, \ \dots \ , \ d_n\) und die Ziffern der resultierenden Zahl \(d_1', \ \dots \ , \ d_n'\).
\\\\
...
Die Uebergabeparameter sind \(d_1, \ \dots \ , \ d_n\) - die Ziffern der gegebenen Zahl - und \(d_1', \ \dots \ , \ d_n'\) - die Ziffern der resultierenden Zahl.
\begin{algorithmic}[1]
    ...
    private HexDigit[] solveImp(int index, int A, int R) {
        ...
        for (HexDigit digit : HexDigitUtils.getHexDigitsDecreasing()) {
            ...
            int[] values = this.hexDigitChanges.getChanges(current, digit).getValues();
						// Amount of segments to add (a) / remove (r) to get from current to digit
            int a = values[0], r = values[1];
    ...
    private void loadEnableDisable() {
        ...
        for (int i = 0; i < this.size; i++) {
            ...
            DigitChanges changes = HexDigitChanges.getSingleton()
															.getChanges(this.given[i], this.result[i]);
...
\begin{lstlisting}[language=Java]
    ...
    private void swapOutside() {
        ...
        while (!isDone) {
            ...
            // Set disable Deque to next non-empty Deque in disable map if needed 
						// and possible (by index)
    ...
    /**
     * Methode for saving swap of two given segments at given (different!) indexes in number 
		 * into changingRow
     *
